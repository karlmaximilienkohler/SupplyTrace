...
**Keyword heuristics.** Substitutability, lead-time, regulatory exposure, and strategic impact are determined by keyword matching on component names and supplier notes. This produces ordinal rankings rather than calibrated cardinal scores, and the match is imperfect. Section 4.1 discusses a specific case where the substitutability keyword heuristic did not correctly capture He-3's uniqueness.
...
SupplyTrace is available at https://github.com/karlmaximilienkohler/SupplyTrace (DOI: 10.5281/zenodo.21105966). The repository includes all source code, quantum cryogenics and photonics taxonomies in JSON format, supplier registries, and trade flow files.
...
**Sample run.** Start the server with `uvicorn app:main --reload` and navigate to `localhost:8000` in a browser. From the dashboard, select the "Quantum Cryogenics" sector, choose the EU perspective, and click "Score". The expected output is a ranked component table with Helium-3 at or near the top, followed by per-pillar breakdowns and a dependency graph. To reproduce the scenario analysis, activate the "Helium-3 Critical Shortage" overlay and observe the EU score increase to approximately 0.844.
...
**Versioning and data availability.** The Zenodo archive (DOI: 10.5281/zenodo.21105966) preserves a complete snapshot of the codebase and data files corresponding to the results in this paper. The GitHub repository may contain subsequent updates.
...
| Geopolitical Dependency | 0.39 | Medium | p2 = 0.2 + (0.75 × 0.20); Russia 20% share, EU risk-relevant jurisdiction set |
...
A known limitation of the current implementation is visible in the substitutability row. The score of 0.30 reflects the keyword-matching heuristic for Pillar 4, which assigns 0.3 to components whose names do not match the specialisation keyword list (Section 2.2). However, He-3 has no functional substitute at dilution refrigerator operating temperatures, a fact that domain knowledge makes unambiguous. A score of 0.30 understates the true substitutability risk and is inconsistent with the qualitative description in this section. This is a recognized limitation of the keyword heuristic approach: the term "helium-3" does not appear in the specialisation keyword list ("custom", "mixing chamber", "superconducting", "niobium", "nbti", "josephson", "dilution"), and so the component does not receive the elevated 0.8 score it should receive on substitutability grounds. Future versions should add "helium-3" to the specialisation keyword list or replace the heuristic with structured expert elicitation.

SupplyTrace shows that from the EU's viewpoint He-3 scores 0.394, from the US viewpoint 0.380, and from China's viewpoint 0.446. The divergence among the three perspectives is 8.1 points, with all of the divergence occurring through the Geopolitical Dependency pillar. China scores highest on the He-3 index simply because the US, which accounts for 55% of He-3 supply, is part of China's risk-relevant jurisdiction set. From both the EU and US viewpoints, Russia's 20% of He-3 production accounts for a moderate geopolitical score of 0.39. From China's viewpoint, its high exposure to the US produces the same factor at 0.65.

When the helium3_shortage scenario overlay is activated (Δ = +0.45), the EU score increases to min(1.0, 0.394 + 0.45) = 0.844. The jump from 0.394 to 0.844 is the primary signal the tool produces: Helium-3 is not merely vulnerable, it is one disruption away from total system collapse for any program that uses dilution refrigerators. As a retrospective plausibility check, this score is consistent with the independently documented structural conditions: SRNS and Rosatom are the only two sovereign He-3 producers, the He-3 market is a de facto duopoly, and physical constraints limit stockpiling by the slow rate of tritium decay at 5.5% annually (Savannah River National Laboratory, 2022).
...
**Keyword heuristics for pillars 4 through 7.** Substitutability, lead-time, regulatory exposure, and strategic impact are determined by keyword matching on component names and supplier notes. This produces ordinal rankings rather than calibrated cardinal scores. The He-3 substitutability score discussed in Section 4.1 is a concrete example of how this heuristic can understate risk for components whose uniqueness is not captured by the current keyword list. Pillar weights were determined by literature consensus, not empirically calibrated against historical disruptions. Future versions should replace keyword heuristics with structured expert elicitation and use the Analytic Hierarchy Process to formally weight pillars.
