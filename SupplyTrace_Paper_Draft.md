...
Quantum computing supply chains exhibit structural vulnerabilities — extreme supplier concentration, low substitutability, and geopolitical exposure — that existing risk tools fail to capture at the component level in a transparent, reproducible way. We present SupplyTrace, an open-source Python framework that computes composite risk scores for individual quantum technology components using a seven-pillar weighted model: market concentration, geopolitical dependency, shelf-life, substitutability, lead-time, regulatory exposure, and strategic impact. Risk is perspective-driven: the same supply chain is evaluated simultaneously from EU, US, and Chinese viewpoints using configurable adversary sets, enabling cross-national divergence analysis. An Accumulated Risk Index (ARI) propagates risk recursively through a bill-of-materials dependency tree, surfacing single points of failure hidden at the sub-component level. Eighteen geopolitical scenario overlays, including export control actions, sanctions regimes, and supply shortages, allow conditional stress-testing of component portfolios. Data is collected through a three-tier hierarchy: UN Comtrade official trade statistics, LLM-assisted web extraction, and AI-estimated prominence rankings, each explicitly labeled by confidence level. Applied to quantum cryogenics and photonics, SupplyTrace correctly identifies Helium-3 and Gallium as the highest-risk components and produces perspective divergence scores consistent with real-world events, including China's July 2023 gallium export restrictions. The software, data, and taxonomies are fully open-source. The methodology and results were informally reviewed by policy officers at the European Commission and a senior quantum technology strategy advisor at TNO.
...
Existing risk tools do not address this at the component level. Government assessments identify strategic dependencies qualitatively but do not score them. Industry consortia map ecosystems without ranking vulnerability. Private platforms monitor at aggregate levels with opaque, technology-agnostic methods. As you move further down the scale of detail toward the component level, the less reproducible and transparent each approach becomes. There is no tool currently available that provides both component-level granularity and full transparency of scoring. Table 1 summarises how supply-chain risks are currently assessed across actor types.
...
| 2 | Geopolitical Dependency | 0.20 | What share of global supply comes from countries considered adversarial by the requesting region? | Trade data and configurable adversary list |
...
**Pillar 2: Geopolitical Dependency.** When trade flow data is available: p2 = min(1, 0.2 + 0.75 × adversary_share), where adversary_share is the total export value controlled by countries in the active perspective's adversary set. The 0.2 baseline accounts for the minimum amount of politically-related risk associated with any dependency. When trade flow data is unavailable, the system falls back to binary evaluation of the suppliers registry: p2 = 0.8 if any supplier's beneficial ownership or operational location is identified as an adversary, and p2 = 0.2 otherwise. Beneficial ownership is evaluated first to account for cases of indirect foreign state control. Adversary sets by perspective: EU adversaries are Russia, China, Iran, Belarus, and North Korea. US adversaries are Russia, China, Iran, North Korea, and Cuba. China adversaries are the US, Japan, South Korea, Taiwan, Australia, the UK, and India.
...
A critical limitation is that HS codes for quantum-specific materials are often coarse. HS code 280429 (Noble Gases) is used for He-3 but includes all noble gases, meaning UN Comtrade data provides a consistent basis for relative ranking but likely misses some defense-related suppliers. AI-generated entries require manual review to remove confabulated entries. Prior to validation, the germanium supplier file contained Samsung, Apple, Toyota, and Saudi Aramco as listed producers. After review, only JSC Germanium (Russia), Umicore (Belgium), Shanghai Germanium, Jiangxi Germanium, and CITIC Metals (China) remained as valid suppliers.

The KNOWN_OWNERSHIP_ALERTS dictionary in extractor.py supplements this by encoding known beneficial ownership relationships that nominal country-of-origin data conceals. For example, CBMM (Brazil) carries a 15% Chinese equity stake via CITIC Metals and Taiyuan Iron and Steel, and Umicore (Belgium) refines germanium but depends entirely on Chinese concentrate supply. These alerts are injected into the geopolitical dependency pillar's scoring function to ensure that a commodity such as niobium, which would score low on adversarial exposure based solely on country-of-origin, is properly scored for being partially controlled by China.
...
SupplyTrace is available at https://github.com/karlmaximilienkohler/SupplyTrace (DOI: 10.5281/zenodo.21105966). The repository includes all source code, quantum cryogenics and photonics taxonomies in JSON format, supplier registries, and trade flow files. Installation requires Python 3.10+ and the dependencies listed in requirements.txt. API keys for Groq (free tier available), UN Comtrade, and optionally OpenAI are loaded via a .env file. The tool runs locally via Uvicorn and is accessed through a standard browser at localhost:8000.

---

## 4. Analysis

This chapter provides evidence of how SupplyTrace works. It is not intended to provide a full overview of all potential quantum supply chain risks, but rather to demonstrate that SupplyTrace can generate credible, differentiated, and externally validatable results. Two commodity flows have been selected: Helium-3 and Gallium. Helium-3 illustrates the seven-pillar model, recursive propagation, and scenario overlay functionality. Gallium demonstrates the perspective-driven architecture and provides an example of external validation. All trade flow estimates in the supporting data files are labeled "AI Estimated". Unless specifically stated, scores were generated using an EU perspective.
...
SupplyTrace scored Helium-3 using UN Comtrade HS code 280429 (Noble Gases), which returned: United States 55%, Russia 20%, Canada 10%, China 5%, Brazil 5%, Japan 5%. The KNOWN_OWNERSHIP_ALERTS dictionary finds no beneficial ownership flags for this product. The resulting seven pillar scores are shown in Table 6.
...
|---|---|---|---|
| Market Concentration (HHI) | 0.46 | Medium | HHI 3,600 from UN Comtrade HS 280429; US 55%, Russia 20% |
| Geopolitical Dependency | 0.39 | Medium | p2 = 0.2 + (0.75 × 0.20); Russia 20% share, EU adversary set |
| Shelf-Life / Perishability | 0.55 | High | Tritium precursor decays at ~5.5% per year; stockpiling physically constrained |
| Substitutability | 0.30 | High | Keyword match: "helium-3", "dilution"; no substitute at 10 to 20 mK |
...
SupplyTrace shows that from the EU's viewpoint He-3 scores 0.394, from the US viewpoint 0.380, and from China's viewpoint 0.446. The divergence among the three perspectives is 8.1 points, with all of the divergence occurring through the Geopolitical Dependency pillar. China scores highest on the He-3 index simply because the US, which accounts for 55% of He-3 supply, is part of China's adversary set. From both the EU and US viewpoints, Russia's 20% of He-3 production accounts for a moderate geopolitical score of 0.39. From China's viewpoint, its high adversarial exposure to the US produces the same factor at 0.65.

When the helium3_shortage scenario overlay is activated (Δ = +0.45), the EU score increases to min(1.0, 0.394 + 0.45) = 0.844. The jump from 0.394 to 0.844 is the primary signal the tool produces: Helium-3 is not merely vulnerable, it is one disruption away from total system collapse for any program that uses dilution refrigerators. External evidence supports the direction of these scores: SRNS and Rosatom are the only two sovereign He-3 producers, the He-3 market is a de facto duopoly, and physical constraints limit stockpiling by the slow rate of tritium decay at 5.5% annually (Savannah River National Laboratory, 2022).
...
SupplyTrace reported the difference between the EU/US and China for gallium at 0.130, the largest of any material in this study. Unlike Helium-3, where China is a net importer, China is the dominant producer of gallium and therefore is not exposed to possible adversaries for access to this material. The Geopolitical Dependency score for both the EU and US for gallium is 0.538. Conversely, China's Geopolitical Dependency score for gallium is 0.312. As expected, the inverse scores are a direct result of the Perspective-Based Architecture: the same supply chain yields different results based on the perspective of the user.

There was external validation of SupplyTrace's high geopolitical score for gallium from a Western perspective from two independent sources. The first was when China imposed export licensing requirements for gallium in July 2023. Analysis by the Stimson Center of Chinese Customs data indicated that gallium exports to the Netherlands and US were almost completely eliminated (Stimson Center, 2025). The second was when the United States Geological Survey modeled the effects of a total ban on gallium and germanium, resulting in a $3.4 billion reduction to US GDP, with more than 40% of this loss attributed to the decline in output in the manufacture of semiconductor devices (Nassar et al., 2024). SupplyTrace's high geopolitical score for gallium from a Western perspective correctly identified the supply chain exposure before the Chinese controls were implemented.
...
**LLM confabulation in the extraction pipeline.** The web extraction pipeline produces false positives for niche industrial materials. Prior to manual review, the germanium supplier file contained Samsung, Apple, Toyota, and Saudi Aramco as listed producers. Manual review removes the most obvious errors but does not scale and introduces reviewer bias. Cross-validation against official business registries (DUNS numbers, OpenCorporates) is the planned mitigation (see Section 6).

**Keyword heuristics for pillars 4 through 7.** Substitutability, lead-time, regulatory exposure, and strategic impact are determined by keyword matching on component names and supplier notes. This produces ordinal rankings rather than calibrated cardinal scores. Pillar weights were determined by literature consensus, not empirically calibrated against historical disruptions. Future versions should replace keyword heuristics with structured expert elicitation and use the Analytic Hierarchy Process to formally weight pillars.
...
First, it generates a purely data-driven score for Helium-3 based solely on trade data and supplier registry information. A baseline score was generated using the helium3_shortage overlay of 0.844 with an overall baseline score of 0.394.
...
The Helium-3 and Gallium case studies demonstrate that the tool functions properly. The scores generated show appropriate divergence for each of the three perspectives (EU, US, and China) and demonstrate alignment with third-party validation, specifically China's 2023 export restriction of gallium. While limitations remain, including issues with false positives in the extraction pipeline, reliance on keyword heuristics for some pillar criteria, and scenario delta values based on expert estimate, these limitations have been identified through confidence labels and source metadata within the tool. The method is new, the software works, and as reviewers at the European Commission and TNO noted, there is a need for it.
